...
- 직원이 직접 만든 추가 후기 요청 템플릿은 발송 이력 유무와 관계없이 ERP의 기존 `삭제`
  ...
  DB 제약에서 삭제를 계속 막는다.
- migration `0062_review_request_template_soft_delete.sql`의 SHA-256은
  `c90859301c6c865e70d20520e2ad84f2d4ee72a9c2a521f5483e039a4c68abca`다. 운영 데이터가 없는
  schema-only 임시 PostgreSQL에서 합성 직원으로 `0061`→`0062`를 적용한 뒤 기본 preset 4개
...
- gateway 162개 테스트, 전체 5패키지 typecheck·lint·production build, DB schema check와
  ...
  digest로 `0061`·`0062`를 순서대로 적용하고 gateway·ERP·홈페이지를 같은 릴리스로 전환한 뒤,
...
- migration `0061_review_request_defaults.sql`은 템플릿 preset·기본 시점과 요청별 추천 문맥을
  추가한다. SHA-256은
...
- core 88개·gateway 161개 테스트, 전체 5패키지 typecheck·lint·production build, DB schema
  ...
  않는다. 운영 반영 시 메인 세션에서 RDS snapshot 뒤 검증한 gateway digest로 `0061`을 먼저
