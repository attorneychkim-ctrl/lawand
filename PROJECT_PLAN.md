# 로앤 통합 플랫폼 — 프로젝트 설계·구현 기준선 (v1.37)
...
> ghost 카드는 사라지게 한다. 전화데스크 상세에서 실제 최종 통화자를 직원이 선택하면
> `staff_resolved` 감사 관계와 final actor를 저장하고 그 뒤에만 최종 직원 후처리를 연다.
...
> migration `0058_tranquil_dark_phoenix.sql`은 고객/사건 연결·담당자·답글·개인 템플릿·요청
...
고객 문자 v1도 같은 gateway 직렬 명령 경계를 사용한다. 전화번호가 수집된 현재 담당 상담에서
텍스트 전용 문자는 센트릭스 `smssend`로 SMS 80바이트·LMS 720바이트 한도를 적용하고,
...
17. 배포된 v0.8.3에서 실제 수신·내선 종료를 통제 관측해 sentinel 0·3분 ring 보정·
