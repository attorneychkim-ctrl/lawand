...
> 표시해 재상담임을 접수 단계에서 바로 알린다. ERP 전역에는 초기
...
> 페이지가 보이는 탭에는 9초 토스트를, 권한을 허용한 브라우저에는 Notification을 보내고
> 둘 다 상담 상세로 연결한다. 시스템 알림은 8초 다중 탭 leader lease와 event ID로 한
> 브라우저에서 중복을 막으며, 상단의 명시적 `알림 켜기`에서 브라우저 권한을 요청한다.
> DB·gateway 이벤트 payload에는 이름·전화·지역을 추가하지 않는다.
...
`invalid(미진입·무효)`로 바꾸며 상담 상태도 종결한다. 표시명 수정 영역 안에는 무효 버튼을
중복 배치하지 않는다.

카카오 이름 접수 트랜잭션은 `consultation.requested`만 남긴다. 전화번호가 없으므로
접수 알림톡을 만들지 않고, 채팅 확인 후 직원이 배정하면
`consultation.kakao_chat.confirmed`와 `consultation.assigned`만 남긴다. 실제 전화번호와 필수
상담정보를 별도 확인하기 전에는 담당 배정 알림톡과
...
- [x] 상담 최초 등록 전 직원 알림: 전역 `consultation.requested` SSE → 보이는 페이지
  토스트·권한 허용 브라우저 Notification·다중 탭 중복 방지·상담 상세 이동
