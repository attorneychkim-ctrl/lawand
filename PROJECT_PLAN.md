# 로앤 통합 플랫폼 — 프로젝트 설계·구현 기준선 (v1.31)
...
> 이 안정화 변경은 코드·인프라 후보만 main에 반영하며 운영 secret·CloudFormation·프로세스는
> 별도 승인된 통합 배포 전까지 변경하지 않는다.
> 같은 날 후속 출시 후보에서는 직원 `신규등록`도 홈페이지와 같은 7일 중복 판정을 적용했다.
...
> 같은 날 후속 후보에서는 카카오 모달에 `선택 · 입력하지 않으셔도 돼요`라고 명시한 휴대전화
