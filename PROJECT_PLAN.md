...
> 누르면 client-only 모달이 `이름 또는 카카오톡 표시명`과 광역 거주지역을 필수로 받고, 확인 시
> same-origin POST가 두 값을 암호화한 전화번호 없는 ERP 상담을 만든 뒤 기존 1:1 채팅
...
> 전화번호만 `010-0000-0000 · 미수집`으로 표시한다. 접수·배정 알림톡은 만들지 않지만,
> `상담하기`는 외부 payload에만 `010-0000-0000`을 합성하고 고객이 선택한 지역과 로그인 직원의
...
### 2-11. 홈페이지 카카오 CTA는 이름·거주지역 입력 후 접수·채팅 확인으로 확정한다

카카오톡 1:1 채팅은 홈페이지 전화 상담 폼의 세 번째 모드가 아니라 독립 접점으로
유지한다. 홈페이지의 카카오 CTA는 외부 채팅 URL을 바로 열지 않고 client-only 모달에서
`이름 또는 카카오톡 표시명`과 광역 거주지역을 모두 필수로 받는다. 고객이 확인할 때 브라우저가 same-origin
`POST /api/kakao-entry`를 새 탭으로 제출하고, 홈페이지 서버가 gateway의
`POST /v1/kakao/homepage-entries`에 이름과 거주지역이 포함된 접수를 저장한 뒤
`https://pf.kakao.com/_AeGxoxl/chat`으로 303 이동시킨다. gateway가 일시적으로
실패하더라도 고객의 채팅 진입을 막지는 않는 fail-open 이동을 사용하고, 실패 로그에는
개인정보를 넣지 않는다. 빈 문자열·공백뿐인 이름 또는 선택하지 않은 거주지역은 확인 버튼과
홈페이지 POST 양쪽에서 거부하고 카카오 채팅방도 열지 않는다. 두 필드를 유효하게 제출한 뒤 gateway가 일시적으로 실패한
...
고객 입력 이름·거주지역과 `표시명_<접수번호 8자리>_플친` ERP 이름은 AES-256-GCM intake에 보존하며,
...
`010-0000-0000`만 합성하고 고객이 선택한 지역을 사용한다. 상담 DB와 이벤트 payload에는 가짜번호를
저장하지 않는다. 배포 전 거주지역이 없던 기존 접수만 외부 등록 시 `미수집`으로 보완한다.
...
- [x] 홈페이지 카카오 CTA → 공백 불허 필수 이름/표시명·광역 거주지역 모달 → 암호화 무전화 ERP 접수 → 1:1 채팅 303 이동
- [x] ERP 입력 이름 즉시 표시 → `상담하기`의 채팅 확인·본인 배정 원자 처리 → 우측 상단 미진입 무효
- [x] 카카오 상담하기 → 가상번호·고객 선택 지역으로 로그인 직원 리걸프렌즈 신건 등록(기존 무지역 접수만 `미수집`),
...
9. 대표 문자 수신함의 U+ 비표준 `SRC` 격리 수정은 배포 완료. 승인된 통제 대표 회선의
   ...
   조건 라우팅은 별도 설계
10. 홈페이지 카카오 버튼 → 이름/표시명·광역 거주지역 입력 → ERP 즉시 표시 → 실제 채팅을 찾아
