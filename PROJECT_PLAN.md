...
> 표시해 재상담임을 접수 단계에서 바로 알린다. 상담 작업 큐에서는 직원이 신규 고객을
...
> 읽으며 누적 원장이나 재통화 큐를 읽지 않는다. 진단 당시 운영 RDS는 `db.t4g.small`이었지만
> 인스턴스 크기를 원인으로 단정하지 않고 쿼리 증폭을 먼저 제거했으며, 사용자 결정에 따라
> RDS 클래스·CloudFormation·운영 데이터는 변경하지 않았다.
...
완전일치 resolver를 한 번 더 적용하고, 일치 건에만 `기존고객` 배지를 표시한다. 목록은
현재 페이지 전화번호를 단일 묶음 조회하며 전화번호 원문을 응답·로그에 추가하지 않는다.
...
- [x] ERP 상담 전화번호의 고객찾기 완전일치 조회 → 목록·상세 `기존고객` 배지
