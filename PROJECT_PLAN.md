# 로앤 통합 플랫폼 — 프로젝트 설계·구현 기준선 (v0.98)
...
템플릿은 `owner_user_id` 기준 직원 개인 설정이며, 소유자 없는 기본 템플릿만 전 직원이 읽고
개인 템플릿은 만든 직원만 조회·수정·사용한다. 이미지 Base64는 저장하지 않고 SOLAPI
스토리지 파일 ID·미리보기 URL·크기·해상도만 보존한다. 고객 번호와 본문은 outbox·로그에
넣지 않고 발송 직전에 기존 암호문에서 복호화하며, 실제 보낸 본문과 제공자 접수 결과는 별도
문자 원장과 상담 상세에서 확인한다. ERP는 개인 템플릿 생성·비활성화, JPG 명함 첨부,
