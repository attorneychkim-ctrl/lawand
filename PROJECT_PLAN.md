# 로앤 통합 플랫폼 — 프로젝트 설계·구현 기준선 (v1.34)
...
> 전·후로 구분하는 상담 묶음 규칙을 구현했다. 7일 안의 정규화 전화번호와 정규화 이름이
> 모두 같은 미종결 상담만 기존 `consultation`에 새 `consultation_request`로 붙인다. 미배정
> 건은 작업 큐에 `배정 전 재요청`, 배정 뒤 건은 `담당 상담 재요청`으로 표시하고 후자는
> 현재 담당자에게만 ERP 실시간·브라우저 알림을 보낸다. 전화만 같거나 이름이 다르면 공유
> 연락처·다른 사건 가능성을 위해 기존처럼 별도 중복 의심 상담을 만든다. 요청별 암호화
> 원문과 광고 귀속은 합치지 않고 그대로 보존한다.
...
> 일반 신규는 같은 정규화 이름·전화의 활성 상담에 요청을 붙이고, 전화만 같으면 별도 중복
> 의심 상담을 만든다. 새 상담에만 최초 접수 알림톡을 만들며 반복 요청에는 중복 발송하지 않는다.
...
제출·같은 세션의 익명→실명 보강·동일 고객 재요청·7일 내 중복 의심을 서로 다른 결과로
처리한다. 7일 안의 정규화 이름과 전화번호가 모두 같고 기존 상담이 종결되지 않았을 때만
동일 고객 재요청으로 기존 상담에 붙인다. 이름이 없거나 다른 공유번호 후보는 자동으로
합치지 않는다. 기존 상담이 미배정이면 `repeat_unassigned`, 담당 배정 이후이면
`repeat_assigned`로 남긴다.
