# 로앤 통합 플랫폼 — 프로젝트 설계·구현 기준선 (v1.33)
...
> 외부 수신 알림은 고객 담당자와 회선 담당자의 합집합, 정확한 담당자가 없으면 활성 직원
> 전체를 대상으로 한다. 리걸프렌즈 담당자는 이름이 아니라 `Member_idx`·`sub_member_idx` →
> `staff_external_accounts.external_member_idx` → ERP 직원 ID로 해석한다. 알림에는 승인된
> 내부 사용자에게 전체 번호·고객·상담/사건·담당자·회선·전달자를 최대한 제공하되,
