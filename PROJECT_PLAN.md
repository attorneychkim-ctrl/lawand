# 로앤 통합 플랫폼 — 프로젝트 설계·구현 기준선 (v1.53)
...
> 2026-08-18에는 리걸프렌즈 담당자가 `무효`로 변경된 상담을 ERP에서 다시 살릴 수 있는
> 복원 경로를 추가했다. 인증 직원이 `상담으로 되돌리기`를 실행하면 기존 담당자 변경의
> 비동기·멱등 원장을 재사용해 리걸프렌즈 사건 담당자와 ERP 상담 담당자를 실행 직원으로
> 함께 변경한다. 외부 변경 성공 전에는 ERP 배정을 앞서 바꾸지 않으며 처리 중 중복 요청과
> 다른 담당자 변경은 기존 pending 제약으로 차단한다. migration과 운영 데이터 변경은 없다.
