...
> 해석한다. 알림에는 승인된 내부 사용자에게 전체 번호·고객명·상담 접수번호/상태·리걸프렌즈
> 사건 종류/상태/번호/사건명/법원·기존 담당자·수신 회선/내선·전달자를 최대한 제공하되,
> 개인정보 없는 변경 신호 뒤 인증 same-origin snapshot에서만 조회한다. 상세 계약과 내선·
...
> 로그인 세션이 same-origin 최소 알림 조회로 고객 입력 이름·전화·거주지역·채널·접수번호를
> 읽는다. 페이지가 보이는 탭에는 지역을 강조한 카드형 10초 토스트를, 권한을 허용한
> 브라우저에는 같은 핵심정보의 Notification을 보내며 시스템 알림도 10초 뒤 닫는다.
> 카드의 `상세 보기`는 상담 상세로 이동하고 `상담하기`는 본인 배정과 전화·카카오의
> 리걸프렌즈 등록 요청을 성공시킨 뒤 상세로 이동한다. 시스템 알림은 브라우저 제약상
> CSS를 적용하지 않고 OS 기본 UI를 사용하되, 고객명 제목과 채널·접수번호·전화·광역지역
> 본문, LAW& 아이콘·배지를 표시한다. 서비스 워커 persistent Notification을 지원하는
> 브라우저에서는 `상담 보기`·`상담데스크` 이동 액션을 제공하고, 미지원 환경은 알림 본체
> 클릭으로 상세에 이동하는 기존 Notification으로 자동 대체한다. 담당자 확정처럼 상태를
> 바꾸는 액션은 네이티브 알림의 오클릭 위험 때문에 넣지 않는다. 상담 ID별 tag로 앞선
...
> 브라우저에서 중복을 막으며, 상단의 명시적 `알림 켜기`에서 브라우저 권한을 요청한다.
...
> 전역 ERP 화면은 전화 상태 카드를 렌더하지 않고 네이티브 브라우저 알림만 유지한다. 외부
...
- [x] 상담 최초 등록 전 직원 알림: 전역 `consultation.requested` SSE → 보이는 페이지
  ...
  상세 이동·토스트 즉시 상담하기. 탭 종료 전달용 Web Push 구독·서버 발송은 후속 범위
