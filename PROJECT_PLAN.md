# 로앤 통합 플랫폼 — 프로젝트 설계·구현 기준선 (v1.56)
...
>
> 2026-08-19 후보: 로앤 홈페이지와 AdPilot이 같은 의미로 광고 성과를 측정하도록
> `lawand-ga4-measurement-v1` 계약을 확정했다. GA4는 명시적으로 분석에 동의한 방문의
> 정제된 페이지뷰와 실제 웹 상담 생성만 관측하며, 맞춤형 광고·Google Signals·광고 관련
> 동의는 사용하지 않는다. 직접 상담과 상담을 생성한 자가진단의 `new`·
> `suspected_duplicate`만 `generate_lead` 후보로 두고, 전체 운영 상담·계약·매출의 기준은
> 계속 gateway·ERP로 유지한다. 네이버 광고·GA4 운영 계정은 아직 연결하지 않았고 코드·
> 개인정보처리방침·운영 설정·배포도 이 설계 확정에서 변경하지 않는다.
> AdPilot 범용 모델·Admin/Data API·goal mapping·`nkw-...` 파서는 조건부 호환이며 상대
> 계약 v1.0.0을 커밋 `784a783659036b668f56204f9e1d8f722f626a1c`로 원격 작업 브랜치에
> 기록했다. 최적화 역할과 상세 보고 목표의 결합, `GA4 전체 발생` 명칭, `리걸프렌즈`
> 고정 문구, 서로 다른 두 키워드 차원의 무음 우선순위는 계정 연결 전 해결할 범용 결손이다.
>
...
- [ ] GA4 운영 속성·웹 스트림 생성과 관리 설정 → 홈페이지 분석 동의·URL 정제·
  직접 상담/자가진단 `generate_lead` 구현 → AdPilot의 최적화 역할/보고 목표 분리·
  범용 연결 문구 → 운영 스트림·키워드 기여 검증
...
GA4 측정의 다음 구현은 [`docs/GA4_MEASUREMENT_V1.md`](docs/GA4_MEASUREMENT_V1.md)의
동의·URL 정제·전환 조건을 홈페이지에 적용하는 작업과, AdPilot에서 아직 연결하지 않은
네이버 광고·GA4 운영 계정을 준비하는 작업으로 분리한다. 계정 연결을 기다리지 않고
Measurement ID가 없으면 완전히 비활성화되는 홈페이지 구현과 범용 AdPilot 계약 검증을
병렬로 진행할 수 있다.
