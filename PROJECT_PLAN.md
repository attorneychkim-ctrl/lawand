# 로앤 통합 플랫폼 — 프로젝트 설계·구현 기준선 (v1.29)
...
> CSS·액션 버튼 없이 클릭 시 상세로 이동한다. 8초 다중 탭 leader lease와 event ID로 한
...
> `0055`는 삭제 시각·삭제 관리자와 종결 일관성 제약을 추가한다. 홈페이지 카카오 CTA는 이름과
...
> 처리 게이트를 거친다. migration `0056`은 카카오 채널의 전화 암호문·지문을 선택값으로 허용한다.
