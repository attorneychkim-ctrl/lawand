# 로앤 통합 플랫폼 — 프로젝트 설계·구현 기준선 (v1.19)
...
2026-08-12 고객 찾기 결과에 `신건상담에 등록` 흐름을 추가했다. 이름·휴대전화·거주
...
외부 사건이나 고객 알림을 만들지 않는다. 출시 후보는 migration `0050`, 전화 디렉터리 재동기화,
gateway·ERP를 같은 릴리스로 적용해야 하며 아직 운영에는 반영하지 않았다.
...
19. 고객찾기 신건·소개 상담 migration `0050`·전화 디렉터리 `living_place` 재동기화·gateway·
    ERP를 같은 운영 릴리스로 배포한다. 소개자 1건과 기존 고객 본인 신건 1건을 통제 등록해
    기본값 수정·상세 문맥을 확인하되 실제 `상담하기`와 리걸프렌즈 신건 생성은 별도 승인
    canary로 분리한다.
