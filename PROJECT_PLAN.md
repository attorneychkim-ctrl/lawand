# 로앤 통합 플랫폼 — 프로젝트 설계·구현 기준선 (v1.10)
...
- [ ] bridge v0.7.2 비식별 관측 로그 통제 배포 → 일반 내선·무조건/통화 후 호전환·
  실패 복귀 양쪽 endpoint 실제 이벤트 canary와 상관키 확정
...
13. bridge v0.7.2 비식별 관측 로그로 두 내선의 일반 통화·무조건/통화 후 호전환·실패
    복귀 canary를 먼저 수행하고 root/leg 상관키를 확정한 뒤, 외부 수·발신 공용 상단 카드와
    담당자 토스트·브라우저 알림·최종 통화자 후처리를 통합 구현
