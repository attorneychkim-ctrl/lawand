# 로앤 통합 플랫폼 — 프로젝트 설계·구현 기준선 (v1.41)
...
링크를 필수로 한다. 한 번에 최대 30개 고객 사건을 선택하고 발송 응답 유실 시 같은 멱등키로
재시도한다. 링크 토큰은 요청 UUID와 서버 HMAC을 묶고 90일·1회 사용으로 제한하며, 입력
전화가 요청 대상 전화와 일치해야 연결된다. 링크가 없는 신규 후기는 전화번호 완전일치 결과가
정확히 한 사건일 때만 자동 연결하고, 복수 후보·이전 후기는 직원이 직접 선택한다. 이름 기반
유사 매칭은 하지 않는다. 연결 당시 리걸프렌즈 담당자 중 활성 ERP 계정이 있는 전원을 답글
의무자로 기록하며, 비식별 실시간 이벤트 뒤 담당자에게만 브라우저 알림을 보낸다. 답글 또는
공개 제한이 끝나면 지속 배지의 의무 건수에서 빠진다.
