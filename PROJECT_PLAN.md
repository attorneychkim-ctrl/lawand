...
개인정보를 넣지 않는다. SSR 첫 렌더에는 모달 input·button을 두지 않고 mount 뒤 portal로
열어 hydration 결정성을 지킨다.
...
배정 전에 직원이 `invalid(미진입·무효)`로 바꾸며 상담 상태도 종결한다.
...
- [x] 홈페이지 카카오 CTA → 이름/표시명 모달 → 암호화 무전화 ERP 접수 → 1:1 채팅 303 이동
- [x] ERP 입력 이름 즉시 표시 → `상담하기`의 채팅 확인·본인 배정 원자 처리 → 미진입 무효
