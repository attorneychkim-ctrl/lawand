...
> 누르면 client-only 모달이 `이름 또는 카카오톡 표시명`을 먼저 받고, 확인 시
> same-origin POST가 그 이름을 암호화한 전화번호 없는 ERP 상담을 만든 뒤 기존 1:1 채팅
...
> `상담하기`는 외부 payload에만 `010-0000-0000`·지역 `미수집`을 합성해 로그인 직원의
...
`상담하기`를 실행해야 본인 배정·리걸프렌즈 신건 등록·담당 배정 알림톡이 시작되므로 생성만으로
외부 사건이나 고객 알림을 만들지 않는다. migration `0050`, 전화 디렉터리 `living_place`
...
`pending(이름 입력 완료·메시지 확인 대기)` 접수다. 같은 브라우저 탭의 30분 이내 반복
...
접수의 DB 전화번호는 실제 `NULL`이고 ERP에서만 `010-0000-0000 · 미수집`으로 표시한다.
고객 입력 원문과 `표시명_<접수번호 8자리>_플친` ERP 이름은 각각 AES-256-GCM 암호화하며,
...
카카오 이름 접수 트랜잭션은 `consultation.requested`만 남긴다. 전화번호가 없으므로
접수 알림톡을 만들지 않고, 채팅 확인 후 직원이 배정하면
...
않는다. 리걸프렌즈에는 실제 전화번호가 없을 때 외부 신건 payload에만
`010-0000-0000`과 지역 `미수집`을 합성하고, 상담 DB와 이벤트 payload에는 가짜번호를
저장하지 않는다. 상담하기 직후 무효를 눌러 사건 연결이 아직 없어도 신건 등록을 먼저
...
- [x] ERP 상담 작업 큐 신규등록: 일반 신규·기존고객·소개건 모달 → 고객찾기 선택 원천 재검증·
  암호화 snapshot → `consultation.requested` 실시간 알림·미배정 `상담하기` 흐름
...
- [x] 홈페이지 카카오 CTA → 공백 불허 필수 이름/표시명 모달 → 암호화 무전화 ERP 접수 → 1:1 채팅 303 이동
- [x] ERP 입력 이름 즉시 표시 → `상담하기`의 채팅 확인·본인 배정 원자 처리 → 우측 상단 미진입 무효
- [x] 카카오 상담하기 → 가상번호·미수집 지역으로 로그인 직원 리걸프렌즈 신건 등록,
...
9. 대표 문자 수신함의 U+ 비표준 `SRC` 격리 수정은 배포 완료. 승인된 통제 대표 회선의
   ...
   조건 라우팅은 별도 설계
10. 홈페이지 카카오 버튼 → 이름/표시명 입력 → ERP 즉시 표시 → 실제 채팅을 찾아
