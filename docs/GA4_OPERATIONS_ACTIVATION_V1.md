...
> 기준일: 2026-08-19
> 상태: Analytics 계정·속성·웹 스트림 및 최소수집 설정 완료, 운영 ID 배포 전
...
Git에 남기지 않고 AdPilot 전용 Orca 세션에 전달했다. 운영 Measurement ID를 홈페이지
secret에 넣어 재배포하기 전이므로 스트림의 수신 데이터가 0건인 것은 정상이다.
...
Best Practices 100, SEO 100이다. 실제 Google 수신과 쿠키 생성·철회는 운영 ID가 홈페이지에
아직 배포되지 않아 실행하지 않았다.
