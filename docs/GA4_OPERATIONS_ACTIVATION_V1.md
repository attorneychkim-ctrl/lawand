...
> 기준일: 2026-08-19
> 상태: 홈페이지·법적 고지 후보 구현, Google 운영 계정 로그인·속성 생성 전
> 측정 계약: [`GA4_MEASUREMENT_V1.md`](GA4_MEASUREMENT_V1.md)
> 원칙: 맞춤형 광고를 사용하지 않고, 분석에 명시적으로 동의한 방문만 측정
...
| 단계 | 작업 위치 | 담당 | 완료 산출물 |
|---|---|---|---|
| 코드·고지 후보 | 이 `GA4_insert` worktree | 홈페이지 세션 | 동의 v2, 개인정보처리방침, 테스트·빌드, 브랜치 커밋 |
| 계정·속성·스트림 | 로앤 소유 Google Analytics 관리 화면 | 로앤 계정 관리자 + 홈페이지 세션 | 계정·속성·스트림 식별자와 설정 검증 기록 |
| 법적 승인 | 로앤 내부 | 김충환 개인정보 보호책임자·책임 변호사 | 동의문·국외이전 고지·Google 약관 수락 승인 |
...
- [ ] Google 로그인 계정이 개인 소유가 아니라 법무법인 로앤이 회수·승계할 수 있는
  조직 관리 계정인지 확인
- [ ] 로앤 구성원 관리자 2명 이상을 확보하고 다중 인증·복구 수단을 설정
- [ ] Google Analytics 약관을 법무법인 로앤을 대리해 수락할 권한이 있는 사람이 확인
- [ ] 개인정보처리방침의 Google LLC, 처리 국가, 항목, 시기·방법, 목적, 보유기간,
  거부 방법·효과를 개인정보 보호책임자가 승인
- [ ] Google 재수탁자와 데이터센터 국가 목록을 활성화 당일 다시 대조하고, 현재 고지의
  `2026년 8월 19일 기준` 목록과 다르면 먼저 고지·동의 버전을 갱신
...
| 통화 | `KRW` |
| 업종 | 실제 법률서비스에 가장 가까운 항목. 없으면 `기타`로 기록 |
| 조직 규모 | 생성 당일 실제 임직원 규모를 사용하고 운영 원장에 선택값 기록 |
| 비즈니스 목표 | 리드 생성과 웹 트래픽·이용행태 파악에 필요한 보고서만 선택 |
...
- [ ] `데이터 스트림 → 웹 스트림 → 향상된 측정` 전체 끔
- [ ] `데이터 수집 → Google Signals` 끔
- [ ] `사용자 제공 데이터 수집`과 사용자 제공 데이터 기능 끔
- [ ] `세부 위치 및 기기 데이터 수집`을 모든 지역에서 끔
- [ ] 광고 개인 최적화·광고 개인화 허용 끔
- [ ] 보고 ID는 `기기 기반` 사용
- [ ] Google Ads, Search Ads 360, Display & Video 360, Firebase, BigQuery 연결 없음
- [ ] User-ID, 잠재고객 공유, 리마케팅, 예측 잠재고객을 만들지 않음
...
- [ ] 웹 스트림의 이메일 주소 가림 켬
- [ ] 아래 금지 쿼리 키를 URL 쿼리 가림 목록에 등록
- [ ] 테스트 화면에서 예시 이메일과 각 쿼리 값이 `(redacted)` 처리되는지 확인
...
- [ ] 같은 `main` commit의 홈페이지 ARM64 이미지를 digest로 배포

### 6-2. 브라우저 canary

새 프로필 또는 사이트 데이터를 지운 지원 브라우저에서 순서대로 확인한다.

1. 최초 진입: 동의 UI가 보이고 `googletagmanager.com`, `google-analytics.com` 요청과
   `_ga` 쿠키가 0개인지 확인한다.
2. 거부: 새로고침·내부 이동 뒤에도 Google 요청과 `_ga` 쿠키가 0개인지 확인한다.
3. 허용: Google 스크립트가 그때 처음 로드되고 최초 페이지의 수동 `page_view`가 1회인지
   확인한다.
4. 내부 이동: URL 변경당 수동 `page_view`가 1회이며 스크롤·클릭·폼 상호작용 등 향상된
   측정 이벤트가 생기지 않는지 확인한다.
5. 합성 URL: `n_keyword_id=nkw-e2e-test&n_query=민감검색어`에서 전자는 남고 후자와 임의
   query·fragment·사례 slug는 GA payload에서 제거되는지 확인한다.
6. 철회: 이후 전송이 중단되고 현재 도메인의 `_ga` 계열 쿠키가 삭제되는지 확인한다.
7. 모바일 Chrome 실기기 새로고침: hydration 경고, 가로 스크롤, 동의창 포커스·스크롤
   문제를 확인한다.
...
1. 운영 홈페이지 환경에서 `LAWAND_GA4_MEASUREMENT_ID`를 제거하고 홈페이지를 안전하게
   재시작해 새 스크립트 로드를 막는다.
2. 배포된 분석 관리자는 ID가 없거나 설정 API가 실패하면 동의를 `denied`로 적용하고 현재
   도메인에서 삭제 가능한 `_ga` 쿠키를 정리한다.
3. AdPilot GA4 동기화와 자동 피드백 입력을 비활성화한다.
4. 영향 기간·이벤트·설정 변경자를 확인하고 내부 사고·변경 원장에 기록한다.
5. 고지·코드·GA 설정을 함께 바로잡고 필요한 경우 동의 버전을 올려 재동의를 받는다.
...
- 홈페이지 동의·국외이전 고지 후보: 구현·로컬 검증 완료
- Google 공식 약관·처리 위치·보유 설정 대조: 완료
- Google 운영 계정 로그인: 미완료
- Analytics 계정·속성·스트림 생성: 미실행
- 운영 Measurement ID 설정·배포: 미실행
- AdPilot 운영 GA4·네이버 광고 연결: 미실행

Orca 내장 브라우저의 Google 로그인은 2026-08-19 현재 `브라우저 또는 앱이 안전하지 않을
수 있습니다`로 거부됐다. 지원되는 일반 브라우저에서 사용자가 직접 로그인한 뒤 이 실행서의
3~5절을 이어서 수행한다. 로그인 실패를 우회하기 위해 비밀번호나 인증 코드를 자동화 도구에
입력하지 않는다.

로컬 후보는 전체 5패키지 typecheck·lint·test·production build를 통과했다. 가짜 형식의
Measurement ID를 쓴 로컬 프로덕션에서 동의 전 Google 요청·쿠키·console 오류가 모두 0임을
확인했고, 390×844 모바일의 접힌/펼친 안내와 1440×1000 데스크톱에서 버튼 표시·가로 스크롤·
hydration 오류가 없었다. 모바일 Lighthouse는 Performance 96, Accessibility 100,
Best Practices 100, SEO 100이다. 실제 Google 수신과 쿠키 생성·철회는 운영 ID가 없어
실행하지 않았다.
