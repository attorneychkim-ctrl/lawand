...
분석을 허용하지 않은 시점에 성공한 제출도 로컬 성공 마커는 먼저 남기고 GA에는 보내지
않는다. 사용자가 나중에 분석을 허용하더라도 과거 성공을 소급 전송하지 않는다.
...
현재 구현 후보는 로컬에서 정제·중복·동의 런타임 단위 테스트, Measurement ID 없음/있음의
동일 빌드 런타임 설정 API, 동의 전 스크립트·쿠키 0, 동의 뒤 정제 페이지뷰, 철회 뒤 전송
