...
> 상태: 2026-08-13 전화번호 기준 상담 묶음과 리걸프렌즈 기존 고객 처리 게이트 구현
...
1. `consultation`은 운영자가 관리할 상담 묶음이고, `consultation_request`는 사용자가
   실제로 누른 제출 한 번이다.
2. 정규화 전화번호가 같고 마지막 요청이 7일 이내이면 채널·입력 이름과 관계없이 같은
   상담 흐름으로 자동 묶는다. 가족·공용 번호는 이름 불일치 경고와 직원 수동 분리로 보정한다.
...
- `consultation_groups`: 활성 묶음의 대표 상담, 전화 HMAC 지문, 최초·최근 요청 시각
- `consultation_group_members`: 별도 채널 상담 원장과 활성 묶음의 1:1 연결
- `consultation_group_events`: 묶기·분리·대표 변경·묶음 병합의 처리자와 비식별 감사 metadata
...
| 4 | 같은 전화 HMAC, 마지막 요청 7일 이내, 기존 상담 미종결 | 이름·채널과 관계없이 대표 상담에 request 부착하거나 별도 채널 상담 원장을 활성 묶음에 연결, 배정 전·후 재요청 구분 | `consultation.request.updated` |
| 5 | 전화 미수집 또는 일치 없음·기존 상담 종결 | 새 독립 상담 생성 | `consultation.requested` |

7일 기준 안에서는 전화번호를 자동 묶음의 단일 고객 키로 사용한다. 입력 이름이 다르면
묶음을 유지한 채 ERP에 경고하고 가족·공용 번호가 확인되면 직원이 구성원을 분리한다.
전화가 없는 플친·네이버 접수는 자동 묶지 않으며 접수번호로 수동 연결할 수 있다. 수동
연결은 서로 다른 전화번호·담당자·리걸프렌즈 사건을 거부하고, 소프트삭제·미진입 무효
상담은 먼저 분리하도록 강제한다. 운영 데이터의 반복 제출 비율을 확인한 뒤 기간을 버전으로
조정한다.
...
과거 계약과 원장 호환을 위해 남긴 이벤트다. v1.33 이후 같은 전화번호의 7일 내 신규 접수는
이름 불일치를 이유로 이 이벤트를 새로 만들지 않고 자동 묶음과 ERP 경고를 사용한다.
...
- [x] 같은 전화의 7일 내 재요청을 이름·채널과 관계없이 배정 전후로 묶고, 담당 배정 뒤
  재요청을 현재 담당자에게만 알린다.
- [x] 목록 대표 1건·상세 전체 요청, 이름 불일치 경고, 전화 없는 접수의 수동 연결·분리와
  묶음 감사 원장, 담당 배정·리걸프렌즈 신건·최초 알림톡 묶음당 1회를 검증한다.
