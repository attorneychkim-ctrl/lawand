...
## vNext 후보 — 상담완료 문자·후기 요청 독립 발송

- 문자 템플릿의 자동발송 조건에 `consultation_completed`를 추가한다. migration
  `0072_consultation_completed_message.sql`은 기존 직원별 조건당 템플릿 하나 제약을 유지하면서
  `message_templates_auto_send_trigger_valid` 허용 목록만 확장한다.
- 통화 후처리에서 `상담완료`를 선택하면 `상담완료 문자발송`과 `후기 요청 문자발송`을 별도
  체크박스로 표시한다. 연결된 템플릿과 리걸프렌즈 고객 사건이 있으면 둘 다 기본 체크하며,
  각 발송의 완료·처리 중·결과 불명 이력이 있으면 해당 항목만 기본 해제해 중복 발송을 막는다.
  이전 발송 실패는 다시 시도할 수 있도록 기본 체크한다.
- 일반 문자와 후기 요청 문자의 `{{고객명}}`은 내부 원장을 바꾸지 않고 발송 치환 시점에만
  앞 3글자를 사용한다. 예를 들어 `김충환_기존`과 `김충환_소개`는 모두 `김충환`으로 발송한다.
- 문자 화면 템플릿 관리 모달은 자동발송 `select`와 JPG 파일 선택 버튼에 다른 입력과 같은
  테두리·배경·focus·hover 스타일을 적용한다.
- 이 후보는 `LegalFlow/sms_consult_complete`의 소스와 migration까지만 범위다. `main` 병합,
  운영 snapshot·migration, gateway·ERP 통합 배포와 실제 고객 발송은 메인 통합 세션에서
  별도로 수행한다.
...
- 허용 변수는 `{{고객명}}`, `{{담당자명}}`, `{{접수번호}}`이며 상담 발송창에서 실제 값으로
  치환한다. `{{고객명}}`은 내부 접미사가 고객 문자에 노출되지 않도록 앞 3글자만 사용한다.
