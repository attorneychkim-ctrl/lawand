...
- 11:20~11:24 KST에 운영 analytics config가 유효한 Measurement ID를 제공하고 응답은
  `private`·`no-store`이며 홈페이지 분석 동의 배너 문구는 0임을 확인했다. 식별자와 요청
  URL은 기록하지 않았다.
- GA4 실시간 보고서에서 로앤 페이지 제목이 정상 표시됐고 최근 30분 활성 사용자 87·최근
  5분 12, `page_view` 136·`session_start` 93·`first_visit` 81·`user_engagement` 62를
  확인했다. 전날의 실시간 0·처리 지연은 해소됐다. 8월 19일까지 일반 보고서도 세션 391·
  활성 사용자 377·신규 사용자 367로 처리됐다.
- 방문 페이지 총 15개를 보고서 두 페이지에서 모두 확인했다. query·fragment·연락처·검색어
  없이 허용된 pathname만 보였고 사례 상세는 `/bank/cases/_detail`로 일반화됐다. 공란 1행은
  있었으나 민감 문자열은 없었다.
- 최근 활동에 운영 `generate_lead`가 이미 감지되어 실제 상담을 추가 생성하거나 합성
  `generate_lead`를 실행하지 않았다. 이 이벤트만 GA4 주요 이벤트로 표시하고 주요 이벤트 탭을
  다시 조회해 등록을 확인했다. 이는 `GA4 관측 리드` 보조 지표이며 primary 자동 최적화 승격이
  아니다. 과거 이벤트가 소급 주요 이벤트로 집계되지 않아 기존 일반 보고서 주요 이벤트 0은
  정상이다.
- Google Ads 링크 완료·승인 필요·요청 전송은 모두 0이며 연결하지 않았다. 맞춤형 광고·광고
  개인화·자동 입찰도 시작하지 않았다. AdPilot 읽기 전용 GA4 OAuth·Admin·Data API 연결은
  별도 `ga4_advertisement_with_lawand_session` 워크트리 세션에 독립 인계했고 네이버 광고
  연결은 다음 승인 게이트다.
- 홈페이지 코드·이미지·운영 배포·DB·상담 데이터는 변경하지 않았다. 이번 확인에서 바뀐 외부
  설정은 GA4 속성의 `generate_lead` 주요 이벤트 표시뿐이며 운영 재배포는 수행하지 않았다.
