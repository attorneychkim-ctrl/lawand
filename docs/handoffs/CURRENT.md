...
1. 후기 요청 템플릿 저장 지연을 재현하고, 저장 요청 timeout·서버 결과 재조회/동기화·명시적
   오류 표시와 개인정보 없는 경로별 duration/status 로그를 추가한다. 같은 시간대 gateway
   request DB pool 포화의 점유 요청을 식별하기 전에는 pool 크기부터 늘리지 않는다.
2. 홈페이지 공개 콘텐츠·운영정보, 개인정보 처리위탁·파기, 실무진 공개 동의를 최종 검수한다.
3. 자가진단·공개 사례의 과거 사건 이용 근거, 희소 조합 일반화, 공개·철회 정책과 책임
   변호사 문구 심사를 완료하기 전에는 자동 공개 범위를 넓히지 않는다.
4. ERP 인증의 이메일 재설정·비활성화 UI·MFA/SSO·외부 rate limit을 설계한다.
...
9. 네이버 예약 기준점 이후 실제 신규 메일 접수 canary와 카카오 운영자 확인 흐름을 별도
   승인 아래 수행한다.
10. 개인 웹훅은 저장·비밀값 보호·재시도·서명·감사·SSRF 방어·이벤트 개인정보 최소화가
   완성되기 전까지 미리보기 상태를 유지한다.
11. OpenAI Realtime STT는 내부 PoC·보관/동의·사람 검토 경계를 확정한 뒤 시작한다.

## 세션 운영 메모

- 현재 작업 관리 기준은 **Orca이며 HERDR가 아니다**. `orca-ide status --json`에서 런타임
  ready/reachable과 앱 `1.4.185`를 확인했고, 현재 Orca 워크트리는
  `LegalFlow/review_save_error`(base `origin/main`, 상태 `in-progress`)다.
